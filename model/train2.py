...
def trainer():
    ...
    headModel = nn.Sequential(
                                ...
                                nn.ReLU(),
                                nn.Dropout(0.5),
                                nn.Linear(256, config.NUMBER_OF_CLASSES)
                            )
    
    model.fc1 = nn.Linear(model.fc.in_features, 1024)
    model.fc2 = nn.Linear(model.fc1.in_features, 512)
    model.fc3 = nn.Linear(model.fc2.in_features, config.NUMBER_OF_CLASSES)
    ...
    with warnings.catch_warnings():
        ...
        for epoch in range(config.EPOCHS):
            ...
            if val_acc > best_val_accuracy:
                ...
                train_val_metrics.to_csv(os.getcwd()+'/Data/train_val_metrics3.csv')
            else:
